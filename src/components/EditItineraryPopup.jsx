  ...
  const fetchCityData = async (city) => {
    ...
    try {
      ...
      if (editingItinerary) {
        ...
        const selectedAttractionsSet = new Set(selectedAttractionsArray.filter(attraction => attractionNames.has(attraction)));
        const selectedActivitiesSet = new Set(selectedActivitiesArray.filter(activity => activityNames.has(activity)));
        const selectedHotelsSet = new Set(selectedHotelsArray.filter(hotel => hotelNames.has(hotel)));

        setSelectedAttractions(selectedAttractionsSet);
        setSelectedActivities(selectedActivitiesSet);
        setSelectedHotels(selectedHotelsSet);
      } else {
              ...
              {hotels.map((hotel) => (
                <div key={hotel.name} className="checkbox-option">
                  <input
                    ...
                    id={hotel.name}
                    value={hotel.name}
                    checked={selectedHotels.has(hotel.name)}
